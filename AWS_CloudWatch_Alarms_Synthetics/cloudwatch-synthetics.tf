...
resource "aws_iam_role_policy_attachments_exclusive" "example" {
  role_name   = aws_iam_role.cw_canary_iam_role.name
  policy_arns = [aws_iam_policy.cw_canary_iam_policy.arn]
}
...
resource "aws_synthetics_canary" "sswebsite2" {
  ...
  runtime_version = "syn-nodejs-puppeteer-9.1" # UPDATED NOV2023
  start_canary    = true
